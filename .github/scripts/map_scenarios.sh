...
ALL=(dotfiles dropbox firefox fzf ghostty moonlander obsidian sysctl system_user timezone ubuntu24-desktop ubuntu24-laptop ulauncher zoxide)
...
for file in "${files[@]}"; do
  case "${file}" in
    ...
    collections/local/ansible_collections/hluaces/*/roles/*/*)
      ...
      role="${role%%/*}"
      case "${role}" in
        sysctl|dotfiles|dropbox|ulauncher|ghostty|fzf|zoxide|firefox|moonlander|timezone|system_user|obsidian)
          picked["${role}"]=1
          ;;
        *)
