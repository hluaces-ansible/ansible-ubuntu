...
if [[ "$INCLUDE_PRE_RELEASE" == true ]]; then
  RELEASE_VERSION=$(wget -O - "https://api.github.com/repos/laurent22/joplin/releases" | grep -Po '"tag_name": ?"v\K.*?(?=")' | sort -rV | head -1)
else
  RELEASE_VERSION=$(wget -O - "https://api.github.com/repos/laurent22/joplin/releases/latest")
  echo "$RELEASE_VERSION"
  RELEASE_VERSION=$(echo "$RELEASE_VERSION" | grep -Po '"tag_name": ?"v\K.*?(?=")')
fi
